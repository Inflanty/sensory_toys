...
/* register four profiles, each profile corresponds to one connection,
...
static void gattc_profile_a_event_handler(esp_gattc_cb_event_t event,
		esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param);
static void gattc_profile_b_event_handler(esp_gattc_cb_event_t event,
		esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param);
static void gattc_profile_c_event_handler(esp_gattc_cb_event_t event,
		esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param);
static void xxx(esp_gattc_cb_event_t event,
		esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param);
...
static const char remote_device_name[4][20] = { "ESP_GATTS_DEMO_a",
		"ESP_GATTS_DEMO_b", "ESP_GATTS_DEMO_c", "ESP_GATTS_DEMO_d" };
...
static struct gattc_profile_inst gl_profile_tab[4] = {
		[PROFILE_D_APP_ID] = { .gattc_cb = gattc_profile_a_event_handler,
				.gattc_if = 1, /* Not get the gatt_if, so initial is ESP_GATT_IF_NONE */
		}, [PROFILE_B_APP_ID] = { .gattc_cb = gattc_profile_b_event_handler,
				.gattc_if = 2, /* Not get the gatt_if, so initial is ESP_GATT_IF_NONE */
		}, [PROFILE_C_APP_ID] = { .gattc_cb = gattc_profile_c_event_handler,
				.gattc_if = 3, /* Not get the gatt_if, so initial is ESP_GATT_IF_NONE */
		}, [PROFILE_A_APP_ID] = { .gattc_cb = xxx,
				.gattc_if = 4, /* Not get the gatt_if, so initial is ESP_GATT_IF_NONE */
		},
...
static void gattc_profile_a_event_handler(esp_gattc_cb_event_t event,
	...
	case ESP_GATTC_REG_EVT:
		...
		printf("MYLOG__REGISTER EVENT COMPLETE : profile a\n");
		break;
	...
	case ESP_GATTC_OPEN_EVT:
		if (p_data->open.status != ESP_GATT_OK) {
			...
			ESP_LOGE(GATTC_TAG, "connect device failed, status %d",
					p_data->open.status);
			conn_device_a = false;
			//start_scan();
			break;
		...
		memcpy(gl_profile_tab[PROFILE_A_APP_ID].remote_bda,
				p_data->open.remote_bda, 6);
		gl_profile_tab[PROFILE_A_APP_ID].conn_id = p_data->open.conn_id;
		ESP_LOGI(GATTC_TAG,
				"ESP_GATTC_OPEN_EVT conn_id %d, if %d, status %d, mtu %d",
				p_data->open.conn_id, gattc_if, p_data->open.status,
				p_data->open.mtu)
		;
	...
	case ESP_GATTC_CFG_MTU_EVT:
		...
		ESP_LOGI(GATTC_TAG, "Status %d, MTU %d, conn_id %d",
	...
	case ESP_GATTC_SEARCH_CMPL_EVT:
		...
		if (get_service_a) {
			...
			if (count > 0) {
				char_elem_result_a = (esp_gattc_char_elem_t *) malloc(
						sizeof(char_elem_result_a) * count);
				if (!char_elem_result_a) {
					...
					status =
							esp_ble_gattc_get_char_by_uuid(gattc_if,
									...
									gl_profile_tab[PROFILE_A_APP_ID].service_start_handle,
									gl_profile_tab[PROFILE_A_APP_ID].service_end_handle,
									remote_filter_char_uuid, char_elem_result_a,
									&count);
					if (status != ESP_GATT_OK) {
						ESP_LOGE(GATTC_TAG,
					...
					if (count > 0
							&& (char_elem_result_a[0].properties
	...
	case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
		if (p_data->reg_for_notify.status != ESP_GATT_OK) {
			ESP_LOGE(GATTC_TAG, "reg notify failed, error status =%x",
		...
		if (count > 0) {
			...
			if (!descr_elem_result_a) {
				...
				if (count
						> 0&& descr_elem_result_a[0].uuid.len == ESP_UUID_LEN_16 && descr_elem_result_a[0].uuid.uuid.uuid16 == ESP_GATT_UUID_CHAR_CLIENT_CONFIG) {
	...
	case ESP_GATTC_SRVC_CHG_EVT: {
		...
		ESP_LOGI(GATTC_TAG, "ESP_GATTC_SRVC_CHG_EVT, bd_addr:%08x%04x",
	...
	case ESP_GATTC_DISCONNECT_EVT:
		...
		start_scan();
		if (memcmp(p_data->disconnect.remote_bda,
				gl_profile_tab[PROFILE_A_APP_ID].remote_bda, 6) == 0) {
			ESP_LOGI(GATTC_TAG, "device a disconnect");
			conn_device_a = false;
			get_service_a = false;

			ESP_LOGI(GATTC_TAG, "ESP_RESTART device A disconnected")
			;
			/* -------------------------------------- RESTART ESP  -------------------------------------- */
			esp_restart();
		}
...
static void gattc_profile_b_event_handler(esp_gattc_cb_event_t event,
		esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param) {
	esp_ble_gattc_cb_param_t *p_data = (esp_ble_gattc_cb_param_t *) param;
	printf("::MYLOG__profile b\n");
	switch (event) {
	case ESP_GATTC_REG_EVT:
		ESP_LOGI(GATTC_TAG, "REG_EVT")
		;
		printf("MYLOG__REGISTER EVENT COMPLETE : profile b\n");
		break;
	case ESP_GATTC_CONNECT_EVT:
	...
	case ESP_GATTC_OPEN_EVT:
		...
		memcpy(gl_profile_tab[PROFILE_B_APP_ID].remote_bda,
				p_data->open.remote_bda, 6);
	...
	case ESP_GATTC_SEARCH_CMPL_EVT:
		...
		if (get_service_b) {
			...
			if (count > 0) {
				char_elem_result_b = (esp_gattc_char_elem_t *) malloc(
						sizeof(char_elem_result_b) * count);
				if (!char_elem_result_b) {
					...
					status =
							esp_ble_gattc_get_char_by_uuid(gattc_if,
									...
									gl_profile_tab[PROFILE_B_APP_ID].service_start_handle,
									gl_profile_tab[PROFILE_B_APP_ID].service_end_handle,
									remote_filter_char_uuid, char_elem_result_b,
									&count);
					if (status != ESP_GATT_OK) {
						ESP_LOGE(GATTC_TAG,
								"esp_ble_gattc_get_char_by_uuid error");
					...
					/*  Every service have only one char in our 'ESP_GATTS_DEMO' demo, so we used first 'char_elem_result' */
					if (count > 0
							&& (char_elem_result_b[0].properties
									& ESP_GATT_CHAR_PROP_BIT_NOTIFY)) {
						gl_profile_tab[PROFILE_B_APP_ID].char_handle =
								char_elem_result_b[0].char_handle;
						esp_ble_gattc_register_for_notify(gattc_if,
								gl_profile_tab[PROFILE_B_APP_ID].remote_bda,
								char_elem_result_b[0].char_handle);
					}
				}
				/* free char_elem_result */
	...
	case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
		...
		esp_gatt_status_t ret_status = esp_ble_gattc_get_attr_count(gattc_if,
				...
				gl_profile_tab[PROFILE_B_APP_ID].service_start_handle,
				gl_profile_tab[PROFILE_B_APP_ID].service_end_handle,
				gl_profile_tab[PROFILE_B_APP_ID].char_handle, &count);
		if (ret_status != ESP_GATT_OK) {
			ESP_LOGE(GATTC_TAG, "esp_ble_gattc_get_attr_count error");
		}
		if (count > 0) {
			...
			if (!descr_elem_result_b) {
				...
				/* Erery char have only one descriptor in our 'ESP_GATTS_DEMO' demo, so we used first 'descr_elem_result' */
				if (count
						> 0&& descr_elem_result_b[0].uuid.len == ESP_UUID_LEN_16 && descr_elem_result_b[0].uuid.uuid.uuid16 == ESP_GATT_UUID_CHAR_CLIENT_CONFIG) {
					ret_status = esp_ble_gattc_write_char_descr(gattc_if,
							gl_profile_tab[PROFILE_B_APP_ID].conn_id,
	...
	case ESP_GATTC_DISCONNECT_EVT:
		if (memcmp(p_data->disconnect.remote_bda,
				gl_profile_tab[PROFILE_B_APP_ID].remote_bda, 6) == 0) {
			ESP_LOGI(GATTC_TAG, "device b disconnect");
			conn_device_b = false;
			get_service_b = false;

			ESP_LOGI(GATTC_TAG, "ESP_RESTART device B disconnected")
			;
			/* -------------------------------------- RESTART ESP  -------------------------------------- */
			esp_restart();
		}
...
static void gattc_profile_c_event_handler(esp_gattc_cb_event_t event,
		esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param) {
	esp_ble_gattc_cb_param_t *p_data = (esp_ble_gattc_cb_param_t *) param;
	printf("::MYLOG__profile c\n");
	switch (event) {
	case ESP_GATTC_REG_EVT:
		ESP_LOGI(GATTC_TAG, "REG_EVT")
		;
		printf("MYLOG__REGISTER EVENT COMPLETE : profile c\n");
		break;
	case ESP_GATTC_CONNECT_EVT:
	...
	case ESP_GATTC_OPEN_EVT:
		...
		memcpy(gl_profile_tab[PROFILE_C_APP_ID].remote_bda,
				p_data->open.remote_bda, 6);
	...
	case ESP_GATTC_SEARCH_CMPL_EVT:
		...
		if (get_service_c) {
			...
			if (count > 0) {
				char_elem_result_c = (esp_gattc_char_elem_t *) malloc(
						sizeof(char_elem_result_c) * count);
				if (!char_elem_result_c) {
					...
					status =
							esp_ble_gattc_get_char_by_uuid(gattc_if,
									...
									gl_profile_tab[PROFILE_C_APP_ID].service_start_handle,
									gl_profile_tab[PROFILE_C_APP_ID].service_end_handle,
									remote_filter_char_uuid, char_elem_result_c,
									&count);
					if (status != ESP_GATT_OK) {
						ESP_LOGE(GATTC_TAG,
								"esp_ble_gattc_get_char_by_uuid error");
	...
	case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
		...
		esp_gatt_status_t ret_status = esp_ble_gattc_get_attr_count(gattc_if,
				...
				gl_profile_tab[PROFILE_C_APP_ID].service_start_handle,
				gl_profile_tab[PROFILE_C_APP_ID].service_end_handle,
				gl_profile_tab[PROFILE_C_APP_ID].char_handle, &count);
		if (ret_status != ESP_GATT_OK) {
			ESP_LOGE(GATTC_TAG, "esp_ble_gattc_get_attr_count error");
		}
		if (count > 0) {
			...
			if (!descr_elem_result_c) {
				...
				/* Erery char have only one descriptor in our 'ESP_GATTS_DEMO' demo, so we used first 'descr_elem_result' */
				if (count
						> 0&& descr_elem_result_c[0].uuid.len == ESP_UUID_LEN_16 && descr_elem_result_c[0].uuid.uuid.uuid16 == ESP_GATT_UUID_CHAR_CLIENT_CONFIG) {
					ret_status = esp_ble_gattc_write_char_descr(gattc_if,
							gl_profile_tab[PROFILE_C_APP_ID].conn_id,
							descr_elem_result_c[0].handle, sizeof(notify_en),
							(uint8_t *) &notify_en, ESP_GATT_WRITE_TYPE_RSP,
							ESP_GATT_AUTH_REQ_NONE);
				}
	...
	case ESP_GATTC_DISCONNECT_EVT:
		if (memcmp(p_data->disconnect.remote_bda,
				gl_profile_tab[PROFILE_C_APP_ID].remote_bda, 6) == 0) {
			ESP_LOGI(GATTC_TAG, "device c disconnect");
			conn_device_c = false;
			get_service_c = false;

			ESP_LOGI(GATTC_TAG, "ESP_RESTART device C disconnected")
			;
			/* -------------------------------------- RESTART ESP  -------------------------------------- */
			esp_restart();
		}
...
static void xxx(esp_gattc_cb_event_t event,
	...
	case ESP_GATTC_REG_EVT:
		ESP_LOGI(GATTC_TAG, "REG_EVT")
		;
		printf("::MYLOG__REGISTER EVENT COMPLETE : profile d\n");
		break;
	case ESP_GATTC_CONNECT_EVT:
	...
	case ESP_GATTC_OPEN_EVT:
		...
		memcpy(gl_profile_tab[PROFILE_D_APP_ID].remote_bda,
				p_data->open.remote_bda, 6);
	...
	case ESP_GATTC_SEARCH_CMPL_EVT:
		...
		if (get_service_d) {
			...
			if (count > 0) {
				char_elem_result_d = (esp_gattc_char_elem_t *) malloc(
						sizeof(char_elem_result_d) * count);
				if (!char_elem_result_d) {
					...
					status =
							esp_ble_gattc_get_char_by_uuid(gattc_if,
									...
									gl_profile_tab[PROFILE_D_APP_ID].service_start_handle,
									gl_profile_tab[PROFILE_D_APP_ID].service_end_handle,
									remote_filter_char_uuid, char_elem_result_d,
									&count);
					if (status != ESP_GATT_OK) {
						ESP_LOGE(GATTC_TAG,
								"esp_ble_gattc_get_char_by_uuid error");
					...
					/*  Every service have only one char in our 'ESP_GATTS_DEMO' demo, so we used first 'char_elem_result' */
					if (count > 0
							&& (char_elem_result_d[0].properties
									& ESP_GATT_CHAR_PROP_BIT_NOTIFY)) {
						gl_profile_tab[PROFILE_D_APP_ID].char_handle =
								char_elem_result_d[0].char_handle;
						esp_ble_gattc_register_for_notify(gattc_if,
								gl_profile_tab[PROFILE_D_APP_ID].remote_bda,
								char_elem_result_d[0].char_handle);
					}
				}
				/* free char_elem_result */
	...
	case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
		...
		esp_gatt_status_t ret_status = esp_ble_gattc_get_attr_count(gattc_if,
				...
				gl_profile_tab[PROFILE_D_APP_ID].service_start_handle,
				gl_profile_tab[PROFILE_D_APP_ID].service_end_handle,
				gl_profile_tab[PROFILE_D_APP_ID].char_handle, &count);
		if (ret_status != ESP_GATT_OK) {
			ESP_LOGE(GATTC_TAG, "esp_ble_gattc_get_attr_count error");
		}
		if (count > 0) {
			descr_elem_result_d = malloc(sizeof(descr_elem_result_d) * count);
			if (!descr_elem_result_d) {
				...
				/* Erery char have only one descriptor in our 'ESP_GATTS_DEMO' demo, so we used first 'descr_elem_result' */
				if (count
						> 0&& descr_elem_result_d[0].uuid.len == ESP_UUID_LEN_16 && descr_elem_result_d[0].uuid.uuid.uuid16 == ESP_GATT_UUID_CHAR_CLIENT_CONFIG) {
					ret_status = esp_ble_gattc_write_char_descr(gattc_if,
							gl_profile_tab[PROFILE_D_APP_ID].conn_id,
							descr_elem_result_d[0].handle, sizeof(notify_en),
							(uint8_t *) &notify_en, ESP_GATT_WRITE_TYPE_RSP,
							ESP_GATT_AUTH_REQ_NONE);
				}
	...
	case ESP_GATTC_DISCONNECT_EVT:
		if (memcmp(p_data->disconnect.remote_bda,
				gl_profile_tab[PROFILE_D_APP_ID].remote_bda, 6) == 0) {
			ESP_LOGI(GATTC_TAG, "device d disconnect");
			conn_device_d = false;
			get_service_d = false;

			ESP_LOGI(GATTC_TAG, "ESP_RESTART device D disconnected")
			;
			/* -------------------------------------- RESTART ESP  -------------------------------------- */
			esp_restart();
		}
		...
		case ESP_GAP_SEARCH_INQ_RES_EVT:
			...
			if (adv_name != NULL) {
				...
				if (strlen(remote_device_name[0]) == adv_name_len
						&& strncmp((char *) adv_name, remote_device_name[0],
								adv_name_len) == 0) {
					...
					break;
				} else if (strlen(remote_device_name[1]) == adv_name_len
						&& strncmp((char *) adv_name, remote_device_name[1],
								adv_name_len) == 0) {
					printf("\n\n\nADV_NAME = DEV_B\nIF = %d\n\n\n",gl_profile_tab[PROFILE_B_APP_ID].gattc_if);
				...
				} else if (strlen(remote_device_name[2]) == adv_name_len
						&& strncmp((char *) adv_name, remote_device_name[2],
								adv_name_len) == 0) {
					printf("\n\n\nADV_NAME = DEV_C\nIF = %d\n\n\n",gl_profile_tab[PROFILE_C_APP_ID].gattc_if);
					...
					break;
				} else if (strlen(remote_device_name[3]) == adv_name_len
						&& strncmp((char *) adv_name, remote_device_name[3],
								adv_name_len) == 0) {
					printf("\n\n\nADV_NAME = DEV_D\nIF = %d\n\n\n",gl_profile_tab[PROFILE_D_APP_ID].gattc_if);
					if (conn_device_d == false) {
						conn_device_d = true;
						ESP_LOGI(GATTC_TAG, "Searched device %s",
								remote_device_name[3]);
						esp_ble_gap_stop_scanning();
						esp_ble_gattc_open(
								gl_profile_tab[PROFILE_D_APP_ID].gattc_if,
								scan_result->scan_rst.bda, true);
...
static void esp_gattc_cb(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if,
	...
	if (event == ESP_GATTC_REG_EVT) {
		if (param->reg.status == ESP_GATT_OK) {
			...
			printf("FAULT__ESP_GATTC_REG_EVT\n");
		} else {
			ESP_LOGI(GATTC_TAG, "Reg app failed, app_id %04x, status %d",
					param->reg.app_id, param->reg.status);
			printf("FAULT__NO_ESP_GATTC_REG_EVT event:%d\n",event);
			return;
		}
	}
